...
const SUFFIXES_PAST = /(ه|اه|وه|واه|ته|تاه|نه|تماه|تموه|توه|تيه|تنه|ناه|هما|اهما|وهما|واهما|تهما|تاهما|نهما|تماهما|تموهما|توهما|تيهما|تنهما|ناهما|هم|اهم|وهم|واهم|تهم|تاهم|نهم|تماهم|تموهم|توهم|تيهم|تنهم|ناهم|ها|اها|وها|واها|تها|تاها|نها|تماها|تموها|توها|تيها|تنها|ناها|هن|اهن|وهن|واهن|تهن|تاهن|نهن|تماهن|تموهن|توهن|تيهن|تنهن|ناهن|ك|اك|وك|واك|تك|تاك|نك|ناك|كما|اكما|وكما|واكما|تكما|تاكما|نكما|ناكما|كم|اكم|وكم|واكم|تكم|تاكم|نكم|ناكم|كو|اكو|وكو|واكو|تكو|تاكو|نكو|ناكو|كي|اكي|وكي|واكي|تكي|تاكي|نكي|ناكي|كن|اكن|وكن|واكن|تكن|تاكن|نكن|ناكن|ني|اني|وني|واني|تني|تاني|نني|تماني|تموني|توني|تيني|تنني|نا|انا|ونا|وانا|تنا|تانا|ننا|تمانا|تمونا|تونا|تينا|تننا|له|اله|وله|واله|تله|تاله|نله|تماله|تموله|توله|تيله|تنله|ناله|لهما|الهما|ولهما|والهما|تلهما|تالهما|نلهما|تمالهما|تمولهما|تولهما|تيلهما|تنلهما|نالهما|لهم|الهم|ولهم|والهم|تلهم|تالهم|نلهم|تمالهم|تمولهم|تولهم|تيلهم|تنلهم|نالهم|لها|الها|ولها|والها|تلها|تالها|نلها|تمالها|تمولها|تولها|تيلها|تنلها|نالها|لهن|الهن|ولهن|والهن|تلهن|تالهن|نلهن|تمالهن|تمولهن|تولهن|تيلهن|تنلهن|نالهن|لك|الك|ولك|والك|تلك|تالك|نلك|نالك|لكما|الكما|ولكما|والكما|تلكما|تالكما|نلكما|نالكما|لكم|الكم|ولكم|والكم|تلكم|تالكم|نلكم|نالكم|لكو|الكو|ولكو|والكو|تلكو|تالكو|نلكو|نالكو|لكي|الكي|ولكي|والكي|تلكي|تالكي|نلكي|نالكي|لكن|الكن|ولكن|والكن|تلكن|تالكن|نلكن|نالكن|لي|الي|ولي|والي|تلي|تالي|نلي|تمالي|تمولي|تولي|تيلي|تنلي|لنا|النا|ولنا|والنا|تلنا|تالنا|نلنا|تمالنا|تمولنا|تولنا|تيلنا|تنلنا|ا|و|وا|ت|تا|ن|تما|تمو|تو|توا|تي|تن)/;
...
class Matcher {
    ...
    static getStems(word) {
        ...
        if (stems.length > 0)
            ...
            for (var i = 0; i < partialsWord.length; i++) {
                ...
                utils.pushUnique(partialsWord, partialsWord[i].replace(/اء$/, //
                    'ا'));
                utils.pushUnique(partialsWord, partialsWord[i].replace(/ى$/, //
                    'ي'));
                utils.pushUnique(partialsWord, partialsWord[i].replace(/ه$/, //
                    'ة'));
            }
        ...
        for (var i = 0; i < candidates.length; i++) {
            ...
            for (var j = 0; j < stems.length; j++) {
                if (stems[j].id == candidates[i].id && stems[j].main == candidates[i].main && stems[j].exact == candidates[i].exact) {
                    found = true;
                    break;
                }
            }
    ...
    static isValidMatch(word, candidate) {
        ...
        if (candidate.posDesc.startsWith('Pres')) {
            stem = stem.replace(/^(ي)/, //
                '');
        } else if (candidate.posDesc.startsWith('Past')) {
            stem = stem.replace(/(.)ا(.)$/, //
                '$1ا?$2');
            stem = stem.replace(/(ا)$/, //
                '($1|ي|)');
        ...
        var m0 = new RegExp('^(?<prefix>' + PREFIXES.source + '*)(?<stem>' + stem + ')(?<suffix>' + SUFFIXES.source + '*)$').exec(word);
        if (m0) {
            if (m0.groups.prefix) {
                var m = null;
                if (candidate.posDesc.startsWith('Pres')) {
                    // pass 3a - verb prefixes are vaid
                    if (!(m = new RegExp('^(?<prefix>(' + PREFIXES_NEG.source + ')?(' + PREFIXES_PRES.source + ')?)(?<stem>' + stem + ')').exec(word)))
                        return false;
                } else {
                    // pass 3b - word prefixes are valid
                    if (!(m = new RegExp('^(?<prefix>(' + PREFIXES_NEG.source + ')?(' + PREFIXES_WORD.source + ')?)(?<stem>' + stem + ')').exec(word)))
                        return false;
                }
                if (m.groups.prefix) {
                    candidate.stem = m.groups.stem
                    candidate.prefix = m.groups.prefix;
                }
            }
            if (m0.groups.suffix) {
                var m = null;
                if (candidate.posDesc.startsWith('Past')) {
                    ...
                    if (!(m = new RegExp('(?<stem>' + stem + ')(?<suffix>(' + SUFFIXES_PAST.source + ')?(' + SUFFIXES_NEG.source + ')?)$').exec(word)))
                        return false;
                } else if (candidate.posDesc.startsWith('Pres')) {
                    // pass 4b - present tense suffixes are vaid
                    if (!(m = new RegExp('(?<stem>' + stem + ')(?<suffix>(' + SUFFIXES_PRES.source + ')?(' + SUFFIXES_NEG.source + ')?)$').exec(word)))
                        return false;
                } else {
                    // pass 4c - word suffixes are valid
                    if (!(m = new RegExp('(?<stem>' + stem + ')(?<suffix>(' + SUFFIXES_WORD.source + ')?(' + SUFFIXES_NEG.source + ')?)$').exec(word)))
                        return false;
                }
                if (m.groups.suffix) {
                    candidate.stem = m.groups.stem;
                    candidate.suffix = m.groups.suffix;
                }
            }
            return true;
        }
