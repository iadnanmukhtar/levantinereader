...
const PREFIXES_WORD = /(ب|بال|ع|عال|ف|فال|ل|لل|ك|كال|و|وب|وبال|وع|وعال|وف|وفال|ول|ولل|وك|وكال|فب|فبال|فل|فلل|ال|وال)/;
const PREFIXES_VERB = /(ف|و)?(ي|ت|ا|ن|بي|بت|ب|بن|من|حي|حت|حا|حن|سي|ست|سا|سن|لي|لت|لا|لن|ماي|مات|ما|مان|مابي|مابت|ماب|مابن|مامن|مي|مت|م|من|مب|مبت|مب|مبن|ممن)/;
        ...
        if (m) {
            if (m[1].length > 0) {
                if (candidate.pos == 'verb') {
                    ...
                    if (!new RegExp('^(' + PREFIXES_VERB.source + ")(" + stem + ')').test(word))
                        return false;
                } else {
