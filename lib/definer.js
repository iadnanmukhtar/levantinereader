...
function getStems(word) {
    ...
    var candidates = findStemCandidates(STOPS, 'stop word', 'term', 'term_b', [word], true);
    if (candidates.length > 0)
        ...
        candidates = findStemCandidates(VERBS, 'present tense', 'pres', 'pres_b', fragsPres);
        if (candidates.length > 0)
        ...
        candidates = findStemCandidates(VERBS, 'past tense', 'past', 'past_b', fragsPast);
        if (candidates.length > 0)
            stemCandidates = stemCandidates.concat(candidates);
        // lookup other words including nouns and adjectives
        candidates = findStemCandidates(STOPS, 'word', 'term', 'term_b', frags);
        if (candidates.length > 0)
            stemCandidates = stemCandidates.concat(candidates);
        candidates = findStemCandidates(WORDS, 'word', 'term', 'term_b', frags);
        if (candidates.length > 0)
            stemCandidates = stemCandidates.concat(candidates);
    }
...
function findStemCandidates(list, pos, colOrig, colMatch, frags, exact) {
    var stems = new Array();
    for (var i = 0; i < list.length; i++) {
        var matches = matchesFrag(list[i][colOrig], list[i][colMatch], frags, exact);
        if (matches.length > 0) {
...
function matchesFrag(termOrig, termMatch, frags, exact) {
    var matches = new Array();
    var origTokens = termOrig.split(/[\\,،/ ]/);
    var termTokens = termMatch.split(/[\\,،/ ]/);
    for (var i = 0; i < frags.length; i++) {
        ...
        for (var j = 0; j < termTokens.length; j++) {
            ...
            if (exact) {
                ...
                var n = getObjectIndexWithValue(matches, 'token', origTokens[j]);
