...
const STOPS = new Dictionary('Stop Word', 'stop', 'terms0');
const WORDS = new Dictionary('General', 'word', 'terms0');
const VERBS_PAST = new Dictionary('Past Tense', 'verb', 'terms0');
const VERBS_PRES = new Dictionary('Present Tense', 'verb', 'terms1');
...
function getStems(word) {
    ...
    for (var i = 0; i < candidates.length; i++) {
        if (!isValidMatch(word, candidates[i])) {
...
function isValidMatch(word, candidate) {
    word = utils.normalizeArabic(word);
    var stem = candidate.partial;
    if (candidate.pos == 'Present Tense')
        stem = stem.substr(1);
    if (candidate.pos == 'Past Tense')
        stem = stem.replace(/(.)ا(.)$/, //
            "$1$2");
    if (word.length == stem.length && word != stem)
        return false;
    var re = new RegExp("^(" + PREFIXES.source + "*)" + stem + "(" + SUFFIXES.source + "*)$");
    if (!re.test(word)) {
        stem = stem.replace(/ى$/, //
            "ي");
        stem = stem.replace(/ة$/, //
        ...
        if (candidate.pos == 'Present Tense')
            stem = stem.replace(/ي$/, //
                "و");
        re = new RegExp("^(" + PREFIXES.source + "*)" + stem + "(" + SUFFIXES.source + "*)$");
        if (!re.test(word))
            return false;
    }
    return true;
